package com.nmate.studycards.kartyamodositscreen.valaszolos
...
@Composable
fun KartyaModositValaszolosScreen(navController: NavHostController, id: Long) {
...
fun Screen(navController: NavHostController, viewmodel: KartyaModositScreenViewModel) {
    ...
    Column() {
        ...
        Box(Modifier
            .padding(8.dp)
            .fillMaxWidth()) {
            valaszKartya(kartya!!.valasz!!, viewmodel)
        }

        Row() {
            Box(Modifier.weight(0.5f)) {
                TextButton(onClick = {
                    viewmodel.updateKartya()
                    navController.popBackStack("KartyaModositValaszt", true)
                    navController.navigate("KartyaModositValaszt")
...
@Composable
fun valaszKartya(text: String, viewmodel: KartyaModositScreenViewModel) {
    Card(Modifier
        .padding(8.dp)
        .fillMaxWidth(), elevation = 8.dp) {
        Box(Modifier.padding(8.dp, 8.dp, 8.dp, 8.dp)) {
            TextField(value = text,
                onValueChange = { viewmodel.setVValasz(it) },
                placeholder = {
